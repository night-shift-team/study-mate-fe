...
type LabelStatus = 'on' | 'off' | 'correct' | 'wrong';
...
export const LabelButton = ({ label, status = 'off' }: LabelButtonProps) => {
  ...
  const baseClasses =
    'flex items-center justify-between p-16 rounded-[12px] font-medium transition-colors min-w-[340px] text-[12px]';

  const statusClasses = {
    on: 'bg-black border border-point-yellow text-white',
